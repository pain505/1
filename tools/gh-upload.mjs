...
// 否则会被拒绝（而且它返回的是含糊的 404，不是 403 —— 很难查）。
// 这里把它单独拎出来，先传其余文件；workflow 缺失时明确提示怎么补。
const WORKFLOW_PREFIX = '.github/workflows/';
const wfFiles = files.filter((f) => f.replace(/\\/g, '/').startsWith(WORKFLOW_PREFIX));
const normalFiles = files.filter((f) => !f.replace(/\\/g, '/').startsWith(WORKFLOW_PREFIX));
if (wfFiles.length) {
  console.log(`\n  注意: ${wfFiles.length} 个 workflow 文件将被暂缓（需要 token 的 workflow 权限）`);
}
